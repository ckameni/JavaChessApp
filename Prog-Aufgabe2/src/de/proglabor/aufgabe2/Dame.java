...
public class Dame extends Figur{
	...
	// Läufer kann bewegt werden, wenn
		...
		if (FarbeDerFigur.equals(amZug) && !startpos.equals(zielpos) && wegFrei && zielPositionVomGegnerBelegt==3){
			return spaltenDifferenz == zeilenDifferenz || spaltenDifferenz == 0 || zeilenDifferenz == 0 ;
	...
	private boolean istDerWegBisZumZielFrei(int startZeile, int startSpalte,
		...
		int zeilenDifferenz = Math.abs(zielZeile - startZeile);
		int spaltenDifferenz = Math.abs(zielSpalte - startSpalte);
		// Wenn Dame mehrere Spalten bzw. Zeilen überspringen muss, um ihr Ziel zu erreichen
				// z.B. Von "A1" nach "A6" oder von "A1" nach "B1"
				if (zeilenDifferenz > 1 || spaltenDifferenz > 1 ){
					// Dame bewegt sich wie Turm
					if (zeilenDifferenz == 0 || spaltenDifferenz == 0 )
						return istDerWegBisZumZielFreiWennSichDameWieTurmBewegt(startZeile,startSpalte,
							 zielZeile, zielSpalte, feld);
					else// Dame bewegt sich wie Laeufer
						return istDerWegBisZumZielFreiWennSichDameWieLaeuferBewegt
						(startZeile,startSpalte,zielZeile, zielSpalte, feld);
				}
				return true;
	}
	/**
 ...
 * @return
 */
	private boolean istDerWegBisZumZielFreiWennSichDameWieTurmBewegt
		(int startZeile, int startSpalte, int zielZeile, int zielSpalte,Figur[][] feld){
		...
		if(startZeile == zielZeile && startSpalte < zielSpalte){ // Spalte inkrementieren und Zeile inkrementieren
			for(j  = startSpalte +1; j < zielSpalte; j++){
				if(feld[i][j]!=null){
					wegFrei= false;
				}
			}
	 ...
	 * @return
	 */
	private boolean istDerWegBisZumZielFreiWennSichDameWieLaeuferBewegt(int startZeile, int startSpalte,
			int zielZeile, int zielSpalte,Figur[][] feld){
	...
	/**
	 *  Funktion ist wichtig um zu prüfen, ob die Dame den gegnerische König 
